...
def download_file_from_link(file_name, file_link):
    """Download a file from a given URL."""
    response = requests.get(file_link)
    print("download_file_from_link: done")
    with open(file_name, "wb") as file:
        file.write(response.content)
    return file_name
# Investment Horizon, Serial Correlation, and Better (Retirement) Portfolios https://bigdata7245.s3.amazonaws.com/staging/pdfs/Investment%20Horizon%2C%20Serial%20Correlation%2C%20and%20Better%20%28Retirement%29%20Portfolios.pdf
def load_multimodal_data_from_s3_file_urls(file_name, url):
    print("load_multimodal_data_from_s3_file_urls: start:")
    print(file_name, url)
    """Download files from S3 URLs and process them."""
    file_path = download_file_from_link(file_name, url)
    print("load_multimodal_data_from_s3_file_urls: done")
    file_extension = os.path.splitext(file.name.lower())[1]
    print("load_multimodal_data:", file_extension)
    with open(file_path, "rb") as file:
        return load_multimodal_data([file])
