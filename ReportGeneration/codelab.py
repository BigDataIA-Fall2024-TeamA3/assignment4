...
# Path to wkhtmltopdf (Ensure this path is correct for your environment)
path_to_wkhtmltopdf = "/usr/local/bin/wkhtmltopdf"  # Adjust as necessary
config = pdfkit.configuration(wkhtmltopdf=path_to_wkhtmltopdf)

# Function to generate markdown content in Codelabs format
def generate_markdown_in_codelabs_format(qa_pairs):
    markdown_content = """
    
    """
    # Loop through the Q&A pairs and add each as a step
    for idx, qa in enumerate(qa_pairs):
        markdown_content += f"""
    ### Step {idx + 1}: Ask a Question and Generate an Answer
    - **Q**: {qa['question']}
    - **A**: {qa['answer']}
...
def main():
    ...
    if st.session_state.qa_history:
        # Export results to Markdown (Codelabs format)
        if st.button("Export Results to Markdown (Codelabs format)"):
            markdown_content = generate_markdown_in_codelabs_format(st.session_state.qa_history)
            # Save the markdown content to a file
            with open("research_results_codelabs.md", "w") as f:
